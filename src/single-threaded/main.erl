...
main_loop(0, NumberOfAnts, NumberOfNodes, Distances, Pheromones, BestScore) ->
  {Pheromones, Distances, BestScore};
main_loop(Iterations, NumberOfAnts, NumberOfNodes, Distances, Pheromones, BestScore) ->
	% io:format("Iterations left: ~w~n", [Iterations]),
  AntsPaths = ant_loop(NumberOfAnts, NumberOfNodes, Distances, Pheromones),
  BestScoreThisIteration = best_path(BestScore, AntsPaths),
  {BestPath, BestDistance} = BestScoreThisIteration,
  io:format("Best distance so far: ~w~n", [BestDistance]),
  NewPheromones = update_pheromones(Pheromones, AntsPaths),
	main_loop(Iterations - 1, NumberOfAnts, NumberOfNodes, Distances, NewPheromones, BestScoreThisIteration).
...
update_pheromone_of_move(Move, Dist, Pheromones) ->
	...
	{From, To} = Move,
    TempPheromones = maps:update_with(Move, Fun, Pheromones),
    maps:update_with({To, From}, Fun, TempPheromones).
...
best_path({}, Paths) -> best_path({[], 0}, Paths);
best_path( BestPathAndDist, []) -> BestPathAndDist;
best_path({BestPath, BestDistance}, [First|Paths]) ->
    {Path, PathDistance, _} = First,
    if  BestDistance=:=0 ->
            best_path({Path, PathDistance}, Paths);
        BestDistance >= PathDistance -> 
            best_path({Path, PathDistance}, Paths);
        true -> 
            best_path({BestPath, BestDistance}, Paths)
    end.

main(Filename) ->
  {Dimension, Map, Distance} = reader:init(Filename),
  Iterations = 100,
  NumberOfAnts = 30,
  {Pheromones, Distances, {BestPath, BestScore}} = main_loop(Iterations, NumberOfAnts, Dimension, Distance),
%   io:format("Pheromones: ~w~n", [Pheromones]),
  io:format("Best path: ~w~nDistance: ~w~n", [BestPath, BestScore]).
